...
import { BookOpen, Search, Filter, Plus, Briefcase } from "lucide-react"
...
export default function PrecedentsView({ firmSlug }: { firmSlug?: string }) {
      ...
      render: (item: Precedent) => (
        <div className="flex flex-wrap gap-1">
          {item.tags.slice(0, 3).map((tag) => (
            <span key={tag} className="inline-flex items-center rounded-lg bg-surface px-2 py-0.5 text-xs text-muted-foreground">
              {tag}
            </span>
          ))}
        </div>
    ...
    },
    {
      ...
      render: (item: Precedent) => (
        <div className="text-sm">
          <p className="text-muted-foreground">{formatDate(item.savedAt)}</p>
          <p className="text-xs text-muted-foreground/70">{item.savedBy}</p>
        </div>
      ),
    },
  ]
