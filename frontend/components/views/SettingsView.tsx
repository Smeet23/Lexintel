...
export default function SettingsView({ firmSlug }: { firmSlug?: string }) {
  ...
  return (
    <>
      ...
      <div className="max-w-3xl space-y-8">
        {/* Theme & Branding - only in firm context */}
        {firmSlug && (
          <SettingCard
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">Customize your firm's appearance in the theme editor</p>
              <Button
                variant="outline"
                onClick={() => router.push(`/firm/${firmSlug}/settings/theme`)}
              >
        ...
        >
          <SettingRow label="Two-Factor Authentication" description="Require 2FA for account access">
        ...
        >
          <div className="flex gap-2">
            <Input value="vrt_sk_••••••••••••••••••••••••" disabled className="font-mono text-sm" />
            <Button variant="outline" size="sm">Regenerate</Button>
          </div>
          <p className="text-xs text-muted-foreground">Last regenerated: January 15, 2026</p>
        </SettingCard>
        ...
        >
          <SettingRow label="Email Notifications" description="Receive weekly usage summaries via email">
            <Switch checked={emailNotifs} onCheckedChange={setEmailNotifs} />
          </SettingRow>
          <Separator />
          <SettingRow label="Query Completion Alerts" description="Notify when long-running queries complete">
