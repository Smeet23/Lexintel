...
export default function DashboardView({ firmSlug }: { firmSlug?: string }) {
  ...
  const matterColumns = [
      ...
      render: (item: MatterResponse) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">{item.file_type.toUpperCase()}</p>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (item: MatterResponse) => (
        <span className="text-[13px] text-muted-foreground">{item.updated_at ? formatRelativeTime(item.updated_at) : formatRelativeTime(item.created_at)}</span>
      ),
    },
  ]
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Button onClick={() => router.push(`${prefix}/matters`)}>
        ...
        >
          <div className="flex items-center justify-between px-6 py-5 border-b border-border">
            ...
            <Button
              ...
              onClick={() => router.push(`${prefix}/matters`)}
            >
          ...
          {isLoading ? (
            ...
            <div className="px-6 py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Create your first matter to get started</p>
            </div>
          ) : (
            <DataTable
